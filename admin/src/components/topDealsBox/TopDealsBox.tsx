...
import { fetchTopUsersPopular } from "../../api/ApiCollection";
...
const TopDealsBox = () => {
  ...
  const { isLoading, isSuccess, data } = useQuery({
    queryKey: ["topuserspopular"],
    queryFn: fetchTopUsersPopular,
  });

  return (
    <div className="w-full p-0 m-0 flex flex-col items-stretch gap-6 xl:gap-4 2xl:gap-9">
      <span className="text-xl xl:text-1xl 2xl:text-2xl font-bold">
        Most popular users
      </span>
      <div className="w-full flex flex-col items-stretch gap-3">
        ...
        {isSuccess &&
          data?.topUsers.map((user: topDealsUser, index: number) => (
            <button
