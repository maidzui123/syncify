...
const SignInForm = (props: signInProps) => {
    ...
    const {register, handleSubmit, formState: {errors}} = useForm<signInDef>({
    ...
    return <motion.form className="flex-1 h-full py-20 px-16" onSubmit={handleSubmit(handleSignIn)}
                        initial={{x: -12, opacity: 0}} animate={{x: 0, opacity: 1, transition: {duration: 1}}}>
        <h1 className="text-3xl font-bold">{t("title:sign_in")}</h1>
        <div className='mt-4'>
            <Label className='mb-1' htmlFor='email'>Email</Label>
            <Input {...register('email')} id='email' type='email' required/>
            <p className="text-red-500 text-xs h-4 align-middle">{errors.email?.message}</p>
        </div>
        <div className='mt-2'>
            <Label className='mb-1' htmlFor='password'>{t("label:password")}</Label>
            <Input {...register('password')} id='password' type='password'/>
            <p className="text-red-500 text-xs h-4 align-middle">{errors.password?.message}</p>
        </div>
        <div className='flex justify-end'>
            <Link to=""
            className='p-0 text-sm text-blue-400 hover:text-red-600'>{t("label:forgot_password") + "?"}</Link>
        </div>
        <Button className='w-full mt-4' type='submit'>{t("button:sign_in")}</Button>
        <div className='mt-4 flex flex-row items-center'>
            <hr className='flex-1'/>
            <div className='mx-4 align-middle leading-none text-sm'>{t("label:or")}</div>
            <hr className='flex-1'/>
        </div>
        ...
        <div className='flex justify-center items-center'>
            ...
            <Button className='p-0 text-blue-400 hover:text-red-600 text-sm'
                    variant='link' onClick={changeForm}>{t("label:sign_up")}</Button>
        </div>
    </motion.form>
}
...
const SignUpForm = (props: signUpProps) => {
    ...
    const {register, handleSubmit, formState: {errors}} = useForm<signUpDef>({
    ...
    return <motion.form className="flex-1 h-full py-8 px-16" onSubmit={handleSubmit(handleSignUp)}
                        initial={{x: -12, opacity: 0}} animate={{x: 0, opacity: 1, transition: {duration: 1}}}>
        <h1 className="text-3xl font-bold">{t("title:sign_up")}</h1>
        <div className='mt-4'>
            <Label className='mb-1' htmlFor='username'>{t("label:username")}</Label>
            <Input {...register('username')} id='username' type='text' required/>
            <p className="text-red-500 text-xs h-4 align-middle">{errors.username?.message}</p>
        </div>
        <div className='mt-1'>
            <Label className='mb-1' htmlFor='email'>Email</Label>
            <Input {...register('email')} id='email' type='email' required/>
        ...
        <div className='mt-1'>
            <Label className='mb-1' htmlFor='confirm_password'>{t("label:confirm_password")}</Label>
            <Input {...register('confirmPassword')} id='confirm_password' type='password'/>
            <p className="text-red-500 text-xs h-4 align-middle">{errors.confirmPassword?.message}</p>
        </div>
        <Button className='w-full mt-2' type='submit'>{t("button:sign_up")}</Button>
        <div className='mt-4 flex flex-row items-center'>
            <hr className='flex-1'/>
            <div className='mx-4 align-middle leading-none text-sm'>{t("label:or")}</div>
            <hr className='flex-1'/>
        ...
        <div className='flex justify-center items-center'>
            ...
            <Button className='p-0 hover:text-red-600 text-sm'
                    variant='link' onClick={changeForm}>{t("label:sign_in")}</Button>
        </div>
    </motion.form>
}
...
const AuthPage = () => {
    ...
    return (
        ...
        >
            ...
            {/*</div>*/}
            <AnimatePresence>
                {newUserStep == AUTH_SCREEN.AUTH && <motion.div
                    className="flex flex-row rounded-xl bg-white h-[580px] aspect-[.85] lg:aspect-[1.7] shadow shadow-gray-500"
                    initial={false} animate={{x: 0, opacity: 1, transition: {duration: 1}}}
                    exit={{x: -999, opacity: 0, transition: {duration: 1}}}>
                    <div
                        className="w-1/2 h-full rounded-xl hidden lg:flex justify-center items-center"
                        style={{backgroundImage: `url(${Resources.backgrounds.LoginBG})`}}>
                        <div className="flex flex-col items-center">
                            <img className="w-1/2 aspect-square bg-contain" src={Resources.logo.sincifyLogo} alt=""/>
                            <h1 className="text-4xl font-bold mt-2 bg-clip-text text-transparent"
                                style={{
                                    background: 'linear-gradient(90deg, rgba(71,100,227,1) 0%, rgba(82,203,212,1) 100%)',
                                    backgroundClip: 'text'
                                }}>Syncify</h1>
                        </div>
                    </div>
                    {formType == 'sign-in' ? <SignInForm changeForm={() => setFormType('sign-up')}/> :
                        <SignUpForm changeForm={() => setFormType('sign-in')}
                                    toOtpScreen={() => setNewUserStep(AUTH_SCREEN.OTP)}/>}
                </motion.div>}
                {newUserStep == AUTH_SCREEN.OTP && <OTPForm handleBack={() => setNewUserStep(AUTH_SCREEN.AUTH)} onDone={() => setNewUserStep(AUTH_SCREEN.INFO)}/>}
                {newUserStep == AUTH_SCREEN.INFO && <NewUserInfo handleBack={() => setNewUserStep(AUTH_SCREEN.AUTH)}/>}
            </AnimatePresence>
        </div>
    );
}
