...
const socketHandler = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("userConnect", async () => {
      ...
      const listRoomChat = await chatServices.getListRoomChat(socket.userId);
  ...
  subClient.on("message", (channel, message) => {
    ...
    if (channel === "userOnline") {
      ...
      io.sockets.sockets
        .get(eventData.socketId)
        ?.to(eventData.chatId)
        .emit(channel, eventData.msgData);
    } else {
...
const sendNotification = async (userId, otherUserId, type, postId) => {
  const payload = await handleSendNotification(
    userId,
    otherUserId,
    type,
    postId
...
const sendMessageChat = async (chatId, socketId, msgData) => {
  ...
  pubClient.publish(
    "chatMessage",
    JSON.stringify({ chatId, socketId, msgData })
  );
};
