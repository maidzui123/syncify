...
//  Get All Posts
router.get("/api/posts", authentication, postControllers.getAllPosts);
...
router.get(
  "/api/posts/replies/:commentId",
  authentication,
  postControllers.getCommentReplies
);

// Get My All Posts Sharing
router.get(
  "/api/posts/sharing/me",
  authentication,
  postControllers.getMyAllPostsSharing
);

// Get User's All Posts Sharing
router.get(
  "/api/posts/sharing/:userId",
  authentication,
  postControllers.getUserAllPostsSharing
);

// Report Post
router.post("/api/posts/report", authentication, postControllers.reportPost);

// Create Assessment
router.post("/api/assessments", authentication, postControllers.createAssessment);
/**
 * @swagger
 * tags:
 *   - name: Post
 ...
 *     description: Trả về danh sách danh sách tất cả bài đăng
 *     parameters:
 *       - name: cursor
 *         in: query
 *         schema:
 ...
 *           type: integer
 *         required: false
 *         description: Số lượng bài đăng trả về (mặc định 10).
 * /api/posts/comment:
 *   post:
 ...
 *     description: "API cho phép người dùng chia sẻ bài đăng."
 *     tags:
 *      - Post
 *     security:
 *       - bearerAuth: []
 ...
 * /api/posts/comments/{postId}:
 *   get:
 *     summary: "Lấy tất cả bình luận của bài đăng"
 *     description: "API cho phép người lấy tất cả bình luận của bài đăng cụ thể."
 *     tags:
 *      - Post
 *     security:
 *       - bearerAuth: []
 ...
 * /api/posts/replies/{commentId}:
 *   get:
 *     summary: "Lấy tất cả trả lời của bình luận"
 *     description: "API cho phép người dùng ấy tất cả trả lời của bình luận cụ thể."
 *     tags:
 *      - Post
 *     security:
 *       - bearerAuth: []
 ...
 *         name: commentId
 *         required: true
 *         description: "ID của bình luận cần lấy trả lời"
 *         schema:
 *           type: string
 *       - in: query
 *         name: cursor
